...
public class TrainController {
	...
	private void getOneConnectionTrains(SearchResult result, List<TrainCapacity> lst, SearchWrapper s, int i) {
		// TODO Auto-generated method stub
	...
	private void getNoneConnectionTrains(SearchResult result, List<TrainCapacity> lst, SearchWrapper s) {
		...
		for(int i = 0; i<lst.size(); i++){
			...
			if(tc.getTrainNumber().endsWith("00")){
				if(expressTrainStations.contains(s.departingStation1) && expressTrainStations.contains(s.arrivalStation1)){}
				else{continue;}
			}
	...
	private int getFare(String trainNumber, Character departingStation, Character arrivalStation) {
		...
		int cost = (t - start)/5;
		if((t-start)%5 != 0){cost++;}
		System.out.println("Cost calculated is **************** " + cost);
	...
	private int getLastDroppingStation(TrainCapacity tc, int from, int to, SearchWrapper s) {
		...
		for(; i<=to; i++){
			...
			try {
				...
				System.out.println("For station" + i + "Current capacity is " + currentCapacity);
