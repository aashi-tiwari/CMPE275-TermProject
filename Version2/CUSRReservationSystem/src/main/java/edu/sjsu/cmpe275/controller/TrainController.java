...
public class TrainController {
    ...
    public ResponseEntity<?> getTrains(@RequestBody String payload) {
		try{
			...
			if((jsonObj.has("exactTime1")  && !jsonObj.getString("exactTime1").equals("")) ||(jsonObj.has("exactTime2") && jsonObj.getString("exactTime2")!="")){
				s.isExactTime=true;
				//If exact time specified, convert the exact time selected into the time format in Train_Capacity table
				if(jsonObj.has("exactTime1")  && jsonObj.getString("exactTime1")!=""){
				...
				if(jsonObj.has("exactTime2")  && jsonObj.getString("exactTime2")!=""){
			...
			if(!s.isRoundTrip){
				...
				if(s.isExactTime && s.ticketType.equalsIgnoreCase("Regular")){
					...
					onewayLst=trainCapacityRepository.searchTrains(direction,s.departingDate1);
					returnLst=trainCapacityRepository.searchTrains(returnDirection,s.departingDate1);
				}
			}
	...
	private void getOneConnectionTrains(SearchResult result, List<TrainCapacity> lst, SearchWrapper s) {
		...
		for(int i = 0; i<lst.size();i++){
			...
			//this is a direct connection found
				...
				SearchResponse response = createResponseObject(tc1.trainNumber, s.departingStation1, s.arrivalStation1, s.departingDate1, s);
				...
				for(int j = nextTrainIndex; j<=8; j++){
					...
					if(finalDroppingStation == to){
						...
						SearchResponse response1 = createResponseObject(tc1.trainNumber, s.departingStation1, (char)(lastDroppingStation + 'A'), s.departingDate1, s);
						SearchResponse response2 = createResponseObject(tc2.trainNumber, (char)(lastDroppingStation + 'A'), s.arrivalStation1, s.departingDate1, s);
